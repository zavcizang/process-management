...
| 姓名 | 角色 | 负责模块 |
|------|------|----------|
| zavci | 架构师 | 项目架构、PCB、进程树、内核核心、系统调用、IPC、日志、交互式 Shell |
| [队友1] | 内核开发 | COW 内存管理器、帧分配器、页面置换算法 |
| [队友2] | 调度开发 | Stride 调度器、优先级调整、溢出处理 |
| [队友3] | 可视化 | 可视化引擎、matplotlib 图表、内存映射图 |
| [队友4] | 测试文档 | 单元测试、集成测试、演示脚本、文档 |
...
本项目在开发过程中使用了 Claude Code（AI 辅助编程工具）进行代码生成和调试。
